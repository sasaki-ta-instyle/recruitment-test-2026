...
export default function AdminReferencePage() {
  // TYPE_MAP のキーは [素直][自他][貢献][ポジネガ] の重要度順なので、降順ソートで意図通り並ぶ
    ...
    <main className="wide-shell">
      ...
      <section className="admin-card">
        ...
        <ol className="ref-steps">
          ...
          <li>
            <strong>絶対 NG ルール</strong> — 素直軸 × 自他軸の両方が負側のとき、自動的に「絶対 NG 該当」フラグを立てる。
          </li>
        </ol>
      </section>
      ...
      <section className="admin-card">
        ...
        <p className="ref-desc">
          記述 10 問を 1 問あたり <b>0〜10 の整数（1 点刻み）</b>で採点（最大 100 点）。面接官が候補者詳細画面で選択した値が DB に保存される。
          下表の 10 / 7 / 4 / 0 はガイド由来のアンカー値で、中間値（1〜3、5・6、8・9 など）も自由に使ってよい。
          <b>無回答／空欄／「特になし」のみ＝0 点</b>、<b>何かしらの記述があれば最低 1 点</b>を原則とする。
        ...
        <p className="ref-desc">
          各設問に対するアンカー値（10/7/4/0）の判定基準、出題意図、AI・テンプレ警戒ポイントを列挙する。中間値も自由に使ってよい。
